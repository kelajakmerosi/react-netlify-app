...
const markNeedsReview = async ({ jobId, parseOutput }: any) => {
  const { rows } = await pool.query(
...
const markFailed = async ({ jobId, error }: any) => {
  const { rows } = await pool.query(
    `UPDATE content_upload_jobs
     SET status = 'failed',
         error_json = $2::jsonb,
...
export default ContentUploadJob
