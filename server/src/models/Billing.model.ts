...
const getPricingCatalog = async () => {
  ...
  const coursePrices = coursesRes.rows.map((row: any) => ({
    id: row.id,
...
const getFinanceSummary = async ({ from, to }: any = {}) => {
  ...
  return {
    ...
    byType: typeRes.rows.map((row: any) => ({
      type: row.payment_type,
      paidCount: Number(row.paid_count || 0),
      revenueUzs: toNumber(row.revenue_uzs),
    })),
    dailyRevenue: dailyRes.rows.map((row: any) => ({
      bucket: row.bucket,
      revenueUzs: toNumber(row.revenue_uzs),
    })),
    range,
