...
const createDraft = async ({ subjectId, ownerUserId, title, description = '', priceUzs = 0 }) => {
...
const addAsset = async ({ packId, storageKey, fileName, mimeType = null, sizeBytes = 0, checksum = null, uploadedBy = null }) => {
  const { rows } = await pool.query(
...
const grantEntitlement = async ({ userId, packId, sourcePaymentId }) => {
  const { rows } = await pool.query(
...
const listLibraryByUser = async (userId) => {
  const { rows } = await pool.query(
    `SELECT
