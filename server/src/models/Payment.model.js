...
const findByProviderExternalId = async ({ provider, externalId }) => {
  const { rows } = await pool.query(
...
const attachCheckoutContext = async ({ paymentId, userId, checkoutContext = {} }) => {
...
const markWebhookStatus = async ({ provider, externalId, status, payload = {} }) => {
  const client = await pool.connect()
  try {
    await client.query('BEGIN')
