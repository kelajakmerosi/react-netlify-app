...
const DATE_TRUNC_BY_GRANULARITY = {
  day: 'day',
  week: 'week',
  month: 'month',
}
...
const getSummary = async ({ from, to, subjectId, userId } = {}) => {
  ...
  const [usersResult, activeResult, completionResult, quizResult, authSourceResult] = await Promise.all([
    ...
    (() => {
      ...
      if (subjectId) {
        ...
        completionWhere.push(`subject_id = $${completionParams.length}`)
      }
      if (userId) {
    ...
    (() => {
      const quizParams = [range.from, range.to]
      const whereClauses = []
      if (subjectId) {
        quizParams.push(subjectId)
        whereClauses.push(`subject_id = $${quizParams.length}`)
      }
      if (userId) {
    ...
    (() => {
      const authParams = [range.from, range.to]
      let userClause = ''
      if (userId) {
  ...
  return {
    ...
    authSources: authSourceResult.rows.map((row) => ({
      source: row.source,
      value: row.value,
    })),
    range,
...
const getTimeseries = async ({ from, to, subjectId, userId, metric = 'active_users', granularity = 'day' } = {}) => {
  const range = buildRange({ from, to })
  const dateTrunc = DATE_TRUNC_BY_GRANULARITY[granularity] || DATE_TRUNC_BY_GRANULARITY.day

  let query = ''
  let queryParams = [range.from, range.to]
  ...
  if (metric === 'user_growth') {
    ...
    if (subjectId) {
      ...
      whereClauses.push(`subject_id = $${queryParams.length}`)
    }
    if (userId) {
      queryParams.push(userId)
...
const getBreakdown = async ({ from, to, subjectId, userId, type = 'subject' } = {}) => {
  const range = buildRange({ from, to })
