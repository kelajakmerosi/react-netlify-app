...
const ContentUploadJob = require('../models/ContentUploadJob.model')
...
const { importExamFromSource } = require('../services/ingestion/importExamFromSource')
const {
...
const parseAllowedQuestionCounts = () => {
  ...
  if (!parsed.length) return new Set([35, DEFAULT_REQUIRED_EXAM_QUESTION_COUNT])
  return new Set(parsed)
}

const ALLOWED_REQUIRED_QUESTION_COUNTS = parseAllowedQuestionCounts()

const inferSourceType = ({ sourceType, sourcePath = '', originalName = '', mimeType = '' }) => {
...
const mapAttemptReasonToError = (reason) => {
...
exports.createTeacherExam = async (req, res, next) => {
  try {
    if (
...
exports.importTeacherExamFromSource = async (req, res, next) => {
...
exports.getTeacherExamImportJob = async (req, res, next) => {
  try {
...
exports.updateTeacherExam = async (req, res, next) => {
  try {
    if (
      req.body.requiredQuestionCount !== undefined
      && !ALLOWED_REQUIRED_QUESTION_COUNTS.has(Number(req.body.requiredQuestionCount))
    ...
    const current = await Exam.getById(req.params.examId)
    if (!current) {
      return sendError(res, {
        status: 404,
        code: ERROR_CODES.EXAM_NOT_FOUND,
    ...
    const updated = await Exam.updateDraft(req.params.examId, req.body)
...
exports.submitTeacherExamReview = async (req, res, next) => {
  try {
    const current = await Exam.getById(req.params.examId)
    if (!current) {
      return sendError(res, {
    ...
    const validation = await Exam.validateExamStructure(req.params.examId)
    if (!validation.valid) {
      return sendError(res, {
        status: 400,
        code: ERROR_CODES.VALIDATION_ERROR,
    ...
    const submitted = await Exam.setStatus(req.params.examId, {
...
exports.getTeacherExamValidation = async (req, res, next) => {
  try {
    const current = await Exam.getById(req.params.examId)
    if (!current) {
      return sendError(res, {
...
exports.getTeacherExamQuestions = async (req, res, next) => {
  try {
    const current = await Exam.getById(req.params.examId)
    if (!current) {
      return sendError(res, {
    ...
    const questions = await Exam.listQuestions(req.params.examId)
...
exports.updateTeacherExamQuestionKey = async (req, res, next) => {
  try {
    const current = await Exam.getById(req.params.examId)
    if (!current) {
      return sendError(res, {
    ...
    const questions = await Exam.listQuestions(req.params.examId)
    const target = questions.find((question) => question.id === req.params.questionId)
    if (!target) {
      return sendError(res, {
        status: 404,
        code: ERROR_CODES.EXAM_NOT_FOUND,
...
exports.approveExam = async (req, res, next) => {
  try {
    ...
    const published = await Exam.setStatus(req.params.examId, {
      status: 'published',
...
exports.rejectExam = async (req, res, next) => {
  try {
    ...
    const rejected = await Exam.setStatus(req.params.examId, {
      status: nextStatus,
      reviewerId: req.user.id,
    })
    if (!rejected) {
...
exports.getExamCatalog = async (req, res, next) => {
  try {
    const exams = await Exam.listPublishedCatalog({ subjectId: req.query.subjectId })
...
exports.createExamCheckout = async (req, res, next) => {
  try {
...
exports.startExamAttempt = async (req, res, next) => {
  try {
    const {
      ...
      reused,
      validation,
    } = await Exam.startAttempt({ userId: req.user.id, examId: req.params.examId })
    ...
    Analytics.trackEvent({
      ...
      payload: { examId: req.params.examId, attemptId: attempt.id },
    }).catch(() => {})

    const questions = (attempt.snapshot?.questions || []).map((question) => ({
...
exports.saveExamAnswer = async (req, res, next) => {
  try {
...
exports.getExamAttemptResult = async (req, res, next) => {
  try {
    const result = await Exam.getAttemptResult({ attemptId: req.params.attemptId, userId: req.user.id })
    if (!result) {
      return sendError(res, {
        status: 404,
        code: ERROR_CODES.EXAM_ATTEMPT_NOT_FOUND,
...
exports.getExamAttemptSession = async (req, res, next) => {
  try {
    const session = await Exam.getAttemptSession({ attemptId: req.params.attemptId, userId: req.user.id })
    if (!session) {
      return sendError(res, {
        status: 404,
        code: ERROR_CODES.EXAM_ATTEMPT_NOT_FOUND,
