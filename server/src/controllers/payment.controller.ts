...
import { isPaymeClickEnabled } from '../config/featureFlags'

const secureEqual = (a: any, b: any) => {
...
const verifyWebhookSignature = ({ req, secret }: any) => {
...
const applyEntitlementFromPaidPayment = async (payment: any) => {
...
const getProviderRequirements = (provider: any) => {
  if (provider === 'payme') {
    return [
...
const buildProviderRedirectUrl = ({ payment, checkoutContext = Object.create(null) as any }: any) => {
...
const resolvePaymentForUser = async ({ paymentId, req, res }: any) => {
  const payment = await Payment.getById(paymentId)
  if (!payment || payment.userId !== (req.user.id as any)) {
    sendError(res, {
      status: 404,
...
export const getPaymentSession = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
export const startPaymentSession = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const payment = await resolvePaymentForUser({ paymentId: req.params.paymentId, req, res })
    if (!payment) return undefined
...
export const confirmPaymentDemo = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
const handleWebhook = async ({ req, res, provider, secret }: any) => {
...
export const handlePaymeWebhook = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
export const handleClickWebhook = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    return await handleWebhook({
      req,
      res,
